...
router.post('/', function(req, res, next) {
  ...
  var { ClubNames } = req.body;
  console.log(ClubNames);
  var query = 'SELECT * FROM Event WHERE ClubName IN (?';
  query += ') OR public=1 ORDER BY EventDate DESC';
  console.log(query);
  req.pool.query(query, [ClubNames], function(err, rows) {
    if (err) {
